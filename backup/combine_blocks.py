...
def offset_lines(lines, dx, dy, suffix, rename=None, is_c=False):
...
def generate(circuit_id, out_dir):
    ...
    x_offset = 0
    gap = 150  # ブロック間の余白
    block_y_max = 0  # 全ブロックのy最大値（ルーティング用）

    prev_output_coords = {}  # 前ブロックのoutputピン座標 {lab: (x,y)}
    prev_outputs = []        # 前ブロックのoutput lab名

    for i, bname in enumerate(selected):
        sch_path = BLOCKS_DIR / f"{bname}.sch"
        if not sch_path.exists():
            print(f"⚠️ {sch_path} not found")
            continue
        ...
        dy = -bounds[1] + 100  # y正方向に

        # ノード名のrename: outputを一意な名前に、前ブロックのoutputを引き継ぐ
        rename = {}
        ...
        # offset適用
        new_n = offset_lines(n_lines, dx, dy, suffix, rename, False)
        new_c = offset_lines(c_lines, dx, dy, suffix, rename, True)
        all_n += new_n
        all_c += new_c

        # ワイヤー接続: 素子エリアの下を通るルーティング
        if prev_output_coords and ports["inputs"]:
            for curr_lab, prev_lab in rename.items():
                if prev_lab in prev_output_coords and curr_lab in pins:
                    ...
                    # y_route: 全ブロックのy_max + margin（素子エリアの下）
                    y_route = block_y_max + 150
                    # 3セグメントのL字ルーティング（下を通る）
                    connecting_wires.append(f"N {x1o} {y1o} {x1o} {y_route} {{lab={prev_lab}}}")
                    connecting_wires.append(f"N {x1o} {y_route} {x2i} {y_route} {{lab={prev_lab}}}")
                    connecting_wires.append(f"N {x2i} {y_route} {x2i} {y2i} {{lab={prev_lab}}}")

        # 今ブロックのoutputピン座標を記録（rename後の名前で）
        prev_output_coords = {}
        for out_lab in ports["outputs"]:
            new_lab = f"{out_lab}_{circuit_id:04d}_{i}"
            if out_lab in pins:
